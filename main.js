...
input.question('Enter latitude: ', (latitudeInput) => {
    ...
    input.question('Enter longitude: ', (longitudeInput) => {
        ...
        const previousDayMoonTimes = SunCalc.getMoonTimes(dateNow.setDate(dateNow.getDate() - 1), latitude, longitude);
        const nextDayMoonTimes = SunCalc.getMoonTimes(dateNow.setDate(dateNow.getDate() + 1), latitude, longitude);
        ...
        const sunProgress = (dateNow - previousSunriseSet) / (nextSunriseSet - previousSunriseSet);
        const moonProgress = (dateNow - previousMoonEvent.date) / (nextMoonEvent.date - previousMoonEvent.date);
