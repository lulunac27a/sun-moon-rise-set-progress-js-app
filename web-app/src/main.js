...
function calculate() {
    ...
    const previousDayTimes = suncalc.getTimes(new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate() - 1), latitude, longitude);
    const times = suncalc.getTimes(dateNow, latitude, longitude);
    const nextDayTimes = suncalc.getTimes(new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate() + 1), latitude, longitude);
    ...
    for (let i = -2; i <= 2; i++) {
        let dateEvent = new Date(dateNow.getFullYear(), dateNow.getMonth(), dateNow.getDate() + i);
        let times = suncalc.getMoonTimes(dateEvent, latitudeInput.value, longitudeInput.value);
        if (times.rise) {
